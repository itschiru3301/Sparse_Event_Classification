#!/usr/bin/env python3
"""Phase 1 & 2: Autoencoder training and classifier fine-tuning."""
...
def train_autoencoder(model, loader, optimizer, n_epochs=20, device=device):
    """Train sparse autoencoder on unlabelled data."""
    model.train()
...
if __name__ == "__main__":
    ...
    print("\n=== Loading Data ===")
    unlabelled_ds = UnlabelledJetDataset(
        configs.UNLABELLED_DATA_PATH,
    ...
    ae_train_loader = DataLoader(unlabelled_ds, batch_size=configs.BATCH_SIZE, shuffle=True, num_workers=4)
    ...
    ae_opt = torch.optim.Adam(ae_model.parameters(), lr=configs.AE_LR, weight_decay=configs.AE_WEIGHT_DECAY)
    ae_history = train_autoencoder(ae_model, ae_train_loader, ae_opt, n_epochs=configs.AE_EPOCHS, device=device)
    
    os.makedirs("models", exist_ok=True)
    torch.save(ae_model.state_dict(), configs.AE_WEIGHTS_PATH)
    print(f"✓ Saved {configs.AE_WEIGHTS_PATH}")
